...
| Ch4-3 SLAM 闭环 | 3 | ☐ | |
| Ch6-1 占据栅格 | 3 | ☑ | 82e2ce2 |
| Ch6-2 ORB 匹配 | 3 | ☑ | 7b1f19f |
| Ch6-3 YOLO | 3 | ☑ | 4f5d9e6 |
| Ch7-1 Reynolds | 4 | ☑ | feat/ch07-swarm |
| Ch7-2 编队 | 4 | ☑ | feat/ch07-swarm |
| Ch8-1 像素到动作 | 4 | ☑ | feat/ch08-learning |
| Video export pipeline | 4 | ☑ | (see 2026-04-26 changelog) |
...
- 2026-04-25 · **Chapter 6 complete** (`feat/ch06-perception`). All three animations landed together on one branch. (a) **Ch6-1 占据栅格 / ESDF** (82e2ce2) — drone serpentines four z-strips at SLICE_Y = 1.2 m over a 16 × 16 m obstacle field; a 72-ray horizontal LIDAR fan rasterises into a 32 × 32 cell grid (0.5 × 0.4 × 0.5 m voxels) at half-voxel step, marking traversed cells as free observed and ray endpoints as occupied. ESDF is computed once on load by brute force (≈ 1000 free × 30 occupied cell pairs ≈ 30 k Euclidean distances) — well within budget. Halfway through the 30 s loop the view flips: every revealed voxel is recoloured by ESDF distance through a four-stop ramp (red 0 m → amber 0.5 m → cyan 1.5 m → blue 2.5 m+) tuned so the densest in-scene clearances (0.3–2.0 m) all land in perceptually distinct bands. Single InstancedMesh of N_CELLS slots, count grows as cells are revealed; per-cell promotion (free → occupied) edits the existing slot's colour rather than allocating a new instance. `logoClouds: false` because the camera orbits at y = 11 looking down on y = 1.2 — clouds would float between the camera and the grid. (b) **Ch6-2 ORB 特征匹配** (7b1f19f) — pure Canvas2D, no Three.js: two side-by-side panels render the same scene description under different affine transforms (right view = pan + 2.6° rotation + 1.03× scale around the panel centre), so audience reads the second image as a slightly displaced camera. Four scenes cycle (street, desk, facade, park); each authors a handful of geometric primitives (rects, circles, lines) and the build step extracts every primitive corner as a keypoint, then applies the right-view transform to compute the *true* correspondence. ~22 % of indices are deterministically scrambled (golden-angle radial offset, clamped to panel) — those are the outliers. 10 s timeline per scene: 0.6–2.0 keypoints stagger-appear, 2.0–4.5 match lines draw, 4.5–5.5 RANSAC fires (outliers flash brighter red on a sin-shaped pulse, then fade out), 5.5–9.4 hold (only inliers), 9.4–10 fade to next scene. Match-line endpoints span the panel gap as world-space-ish polylines on a single overlay canvas covering the whole stage. (c) **Ch6-3 YOLO 目标检测** (4f5d9e6) — small "city block" 3-D scene (5 buildings as scenery, 4 cars + 5 pedestrians + 2 signs as detection targets) with a 56° FOV camera orbiting at radius 13, altitude 6.5 with a 0.7 m sinusoidal bob. Each target carries an explicit `halfExtents` AABB; per frame we build `matVP = projectionMatrix · matrixWorldInverse` and project all 8 corners through `Vector4.applyMatrix4`, taking min/max over the homogeneous-w-validated NDC for the screen-space bounding box. Important: `camera.updateMatrixWorld()` does not refresh `matrixWorldInverse` (only `renderer.render` does), so we invert manually before computing matVP — without this the overlay lags the 3-D frame by one tick. Confidence per target = baseConf (0.78–0.92) + small two-sine jitter + a single 0.4 s soft-cosine dropout per loop seeded from the target index — most targets visibly drop below the 0.45 threshold once and reappear, teaching the per-frame nature of detection. Camera-feed chrome (corner brackets + centre crosshair) is drawn on the overlay above the bbox layer to sell the "onboard camera" framing. CapsuleGeometry for pedestrians is fine on the latest Three.js (note in CLAUDE.md re r128).
- 2026-04-26 · **Chapter 7 (P1) complete** (`feat/ch07-swarm`). Both planned P1 swarm animations landed on one branch. (a) **Ch7-1 Reynolds 集群** (`01-reynolds.html`) — 36 dart instances in a 24×9×24 m bounded volume implementing the classical three-rule model: separation (1/d²-weighted pushback inside SEPARATION_R = 1.5 m), alignment (mean neighborhood velocity inside PERCEPTION_R = 3.2 m), cohesion (steer toward neighborhood centroid, same radius). Per-rule steering force clipped to MAX_FORCE = 7.5 m/s², integrated velocity capped at MAX_SPEED = 4.5 m/s — without the speed cap, "仅聚合" rapidly bounces drones off the soft quadratic walls at unreadable speeds. A slow Lissajous-driven virtual goal (amber sphere + halo) keeps the flock circulating so "仅对齐" doesn't collapse to a static drifting line. The focus drone (index 0, executed/magenta) carries two nested translucent spheres — outer cyan = perception, inner magenta = separation — so the audience reads exactly which neighbors each rule consults. Per-instance velocity-aligned quaternion via `setFromUnitVectors(Vector3(0,0,1), velNormalized)` on a +z-pointing cone (`rotateX(π/2)` baked at build time so the apex is along formation forward, not the cone's default +y). O(N²) pairwise — 1296 distance checks per frame at N = 36, no spatial hash needed. Three sliders (0–2.5) with thumb colors keyed to the legend semantics (sep=magenta, ali=amber, coh=green) plus four presets (均衡 / 仅分离 / 仅对齐 / 仅聚合); the "only X" presets keep a residual sep = 0.3 because the teaching point is each rule's *contribution*, not the degenerate edge case of zero separation stacking all drones onto a single point. Subtle gotcha during build: an early version of the steering helper returned a shared `_steer` object, so reading `addSteering(...).x + addSteering(...).y + addSteering(...).z` summed the same-call value thrice — replaced with single-call-then-read-fields. (b) **Ch7-2 编队飞行** (`02-formation.html`) — leader-follower at N = 8. Leader (drone 0, control/amber, scale 1.0) tracks an analytical 3-D Lissajous (R = 8.5 m, height bob ±1.2 m around y = 5, figure-8 in xz) with both position AND velocity given in closed form so the follower yaw frame and FF term are always smooth — finite-difference velocity would jitter on direction reversals. Yaw quaternion via `setFromUnitVectors((0,0,1), heading_xz_unit)`, cached when |vel_xz| < 1e-2 so a near-stop doesn't snap orientation. Each of the 7 followers carries a slot offset in the leader's body frame and runs PD on position with leader-velocity feedforward (KP = 6, KD = 4.5, KV_FF = 0.95, accel cap 14 m/s²) — without FF the followers visibly trail off the back at constant cruise; with FF the steady-state offset collapses and the only visible error is during shape morphs. Three formation tables — 菱形 (3-D diamond), 一字 (line-astern, 1.8 m spacing), 网格 (3-2-3 wedge). The "网格" is a wedge rather than literal 3×3 because 3×3 = 9 slots and 7 followers leaves an audience-confusing missing tooth. Shape morph: 1.4 s smoothstep lerp from a snapshot of the current *active* slots (not the previous shape's table) to the new target — chained mid-morph clicks don't snap. Wireframe ghost cones at each commanded slot read as "intent" against the solid cyan drones reading as "actual"; HUD max/mean tracking error quantifies the gap. Followers init at frame 0 to their slot world positions with leader velocity already in v, so the demo starts in formation rather than spending a half-second collapsing into one.
- 2026-04-26 · **Video export pipeline** landed (`tools/record/`). Playwright drives every animation through a deterministic frame loop: an `addInitScript` shim replaces `performance.now`, `Date.now`, and `requestAnimationFrame` with a fake clock that advances exactly `1000/fps` ms per `__recorder.step()`, so output is reproducible regardless of host CPU load. `#stage` is screenshotted each tick (PNG over CDP) and piped straight into `ffmpeg-static` (libx264, CRF 18, faststart) — no temp frame dir. Captures canvas *and* HUD overlays, which canvas-only `MediaRecorder` would miss. `manifest.json` lists all 19 animations with optional per-entry `duration`/`fps` overrides plus `variants` carrying CSS click selectors for the 4 interactive scenes (Ch2-1 stage toggle, Ch3-2 PID presets, Ch7-1 Reynolds presets, Ch7-2 formation shapes) — each variant emits its own clip from a known initial state. 2-worker pool by default; 27 light-mode clips encoded in 10.4 min on this box. Windows-specific gotcha: `viteProc.kill()` only signals the immediate cmd.exe child, leaving npx → vite grandchildren bound to port 5173 and the parent Node hanging — driver now uses `taskkill /T /F` plus an explicit `process.exit(0)` after the worker pool drains. Added `README.md` at the project root and the `out/` ignore rule. New scripts: `npm run record` (with `--only`/`--theme`/`--workers`/`--fps`/`--duration`/`--no-vite` flags). New deps: `playwright`, `ffmpeg-static`.
- 2026-04-26 · **Ch8-1 像素到动作** landed (`01-pixels-to-action.html`). Single full-screen stage split horizontally: top 58 % is a 3-D scene where a drone navigates a 6-cylinder field cycling through 4 corner goal beacons under a scripted potential-field controller; bottom 42 % is a single overlay-canvas schematic of an end-to-end policy network drawn left → right (32 × 24 grayscale observation → 4 conv1 maps → 8 conv2 maps → 32-cell FC → 4 bipolar action bars labelled v_z / v_x / v_y / ω_ψ), with soft Bezier connectors fanned between adjacent columns. **The honesty trick:** the drone is steered by the script, but the conv/fc activations are real — luma is read back from a 32 × 24 `WebGLRenderTarget` of the onboard camera each inference tick (30 Hz), conv1 applies four hand-coded 3 × 3 kernels (H-Sobel, V-Sobel, diagonal, Laplacian / blob) for `|response|` edge magnitudes, conv2 mixes them through 8 fixed-RNG 3 × 3 × 4 kernels at stride 2 + ReLU, FC projects the 1536-D flatten through a fixed 1536 × 32 `1/√fan-in` random matrix + ReLU. The activations therefore *react* to what the camera sees (goal blobs, obstacle silhouettes) without driving anything; the bottom note carries the disclaimer (`示意网络 · 真实端到端策略训练自仿真器, 此处推理由脚本生成`). Render order each frame: onboard view → 32 × 24 RT (drone hidden so its own body doesn't fill the view) → full-canvas clear → scissored top-region external orbit → schematic overlay. Two iteration fixes after first paint: (a) **dark-mode observation invisibility** — the scene's near-black `theme.colors.bg` made luma cells render as black-on-black; added a theme-aware display lift `disp = luma · 0.78 + 0.20` (dark) / `· 0.86 + 0.04` (light) so the panel always reads as a low-res camera display, while the conv pipeline keeps consuming the raw luma; (b) **drone stalled inside the obstacle field** — pure radial repulsion has local minima where two obstacles' pushback vectors cancel, so the policy now adds a 90°-rotated tangent component (sign chosen via `tangent · goalDir ≥ 0`) at `0.9 · radial + 0.8 · tangent` weight, and the start position moved to `GOALS[0]` facing -Z so the first leg is the perimeter run rather than the diagonal across the central obstacles. `mountThemeToggle()` toggle stays clear of the action-bars column thanks to a 36 px right pad and the bipolar bar layout (numerics anchored 26 px before the column right edge).
